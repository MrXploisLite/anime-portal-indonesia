...
export default function OtakudesuCompletedPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await otakudesuApi.getCompleted(page);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-4">Anime Completed - Otakudesu</h1>
        <p className="text-gray-400">Anime yang sudah tamat dari Otakudesu</p>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
            {animeList.map((anime, index) => (
              <AnimeCard key={anime.slug || anime.id || `anime-${index}`} anime={anime} source="otakudesu" />
            ))}
          </div>
