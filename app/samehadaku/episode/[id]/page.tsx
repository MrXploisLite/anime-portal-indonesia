...
export default function SamehadakuEpisodePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await samehadakuApi.getEpisode(id);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="glass-effect rounded-lg p-6 mb-8">
        ...
        {episode.animeId && (
          <Link
            href={`/samehadaku/anime/${episode.animeId}`}
            className="inline-block px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg transition mb-6"
          >
        ...
        <div className="glass-effect rounded-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {episode.streamServers.map((server, index) => (
              <Link
                key={index}
                href={server.url || `/samehadaku/server/${server.id}`}
      ...
      {episode.prevEpisode && (
        <div className="mt-8 flex justify-between">
          <Link
            href={`/samehadaku/episode/${episode.prevEpisode.slug || episode.prevEpisode.id}`}
          ...
          {episode.nextEpisode && (
            <Link
              href={`/samehadaku/episode/${episode.nextEpisode.slug || episode.nextEpisode.id}`}
              className="px-6 py-3 bg-purple-600 hover:bg-purple-700 rounded-lg transition"
            >
              Episode Selanjutnya →
            </Link>
